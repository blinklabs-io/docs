...
Version 0.1.15 keeps Plutigo portable across supported Go targets, including 32-bit environments. The `unConstrData` overflow check now compares through `uint64`, which avoids platform width overflow while keeping the earlier 64-bit overflow behavior.
