...
Leios is an experimental node to node protocol suite in gOuroboros. In v0.179.0, the suite adds `LeiosVotes` as protocol ID `20`. `LeiosNotify` handles block announcements and availability notifications, `LeiosFetch` handles block and transaction retrieval, and `LeiosVotes` handles vote diffusion as part of the CIP-0164 / Linear Leios work.
