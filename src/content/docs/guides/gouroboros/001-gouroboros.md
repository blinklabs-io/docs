...
Leios is an experimental node-to-node mini-protocol suite in gOuroboros. In v0.179.0, the suite adds `LeiosVotes` as protocol ID `20`. The suite now splits responsibilities across `LeiosNotify` for block announcements and availability notifications, `LeiosFetch` for block and transaction retrieval, and `LeiosVotes` for vote diffusion as part of the CIP-0164 / Linear Leios work.
