...
* Added **built in Leios network support:** gOuroboros now includes a built in `NetworkCardanoLeios` entry with the Leios testnet network magic and bootstrap peer, so clients can resolve and connect to Leios through the standard network registry APIs instead of building network configs by hand.
...
* Improved **ledger voting procedure helpers:** Value semantics helper methods such as `AddOrReplace`, lookup helpers, and clone and equality support now prevent duplicate logical votes caused by address based vote tracking while preserving existing CBOR encoding.
