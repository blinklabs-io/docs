...
This release adds the new Conway era `GetAccountState` query, simplifies ledger era handling by treating Leios as Dijkstra era overlay functionality instead of a standalone era, and fixes pipeline validation so unchecked blocks do not affect chain state when validation is enabled.
