...
* Enforced **phase 2 fail closed behavior:** Transactions that require unsupported phase 2 validation now stop with an explicit error instead of appearing valid.
...
* Recorded **duplicate map key rejection:** Duplicate encoded map keys now fail decoding early, which makes malformed payload handling more predictable for validators and integrators.
