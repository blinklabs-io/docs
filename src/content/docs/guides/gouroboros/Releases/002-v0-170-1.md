...
* Guarded **unsupported valid script paths with an explicit error:** Transactions marked valid now stop with a clear error when they would require a validation step that this path cannot perform.
...
* Recorded **collateral return safety follow through:** The release pairs the collateral return fix with stronger validation and regression coverage so the corrected behavior stays protected.
