...
* Corrected **unsupported handshake version errors:** Unsupported handshake versions now fail with a clear error path instead of producing less precise handling.
...
* Tightened **expired DMQ notification draining:** Notification draining now filters expired messages so stale distributed messages do not continue through the drain path.
...
* Recorded **duplicate map key rejection:** Duplicate `CBOR` map keys now fail decoding early, which makes malformed payload handling more predictable for validators and integrators.
