...
This release improves raw block delivery when a typed decoder cannot represent a wire layout, limits script data hash checks to scripts needed by transaction purposes, gates selected checks for phase-two-invalid transactions, and validates explicitly supplied treasury values.

### 💪 Improvements

* Improved **raw block delivery:** BlockFetch now delivers the original block bytes to `BlockRawFunc` when typed decoding cannot represent the wire layout. Block validation, point and previous-hash range correlation, continuity checks, and typed consumer behavior remain intact. Typed nil decode errors no longer cause panics. This supports Conway-tagged Dijkstra-shaped blocks, including Musashi payloads.
