...
* Updated **transaction validity handling:** decoding transaction CBOR now preserves lower and upper bounds, including explicit zero-valued bounds, and Plutus validity ranges now match `cardano-ledger` upper-bound semantics. Test coverage expanded with `ouroboros-mock` validity fixtures, and dependencies now point to `ouroboros-mock` `v0.15.1` and `plutigo` `v0.3.0`.
