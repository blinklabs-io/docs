...
* Hardened **pool relay decoding:** `PoolRelay.UnmarshalCBOR` rejects IPv4 address widths other than `4` bytes, IPv6 address widths other than `16` bytes, and null `dns_name` values for hostname relay types. It returns `ErrPoolRelayAddressWidth` or `ErrPoolRelayMissingHostname` for these invalid values.
