...
* Aligned **Cardano data ingestion:** Adder now uses `gouroboros` v0.193.0.
...
* Strengthened **Cardano validation:** Adder now applies stricter decoding and validation rules while incorporating KES and ledger fixes.
...
* Accelerated **Plutus processing:** Adder benefits from hardened decoding and evaluation together with performance improvements in its Plutus integration.

* Protected **image handling:** Adder now uses hardened image decoders to process image data more safely.

### 🧪 Testing, CI, Docs, and Release Tooling

* Upgraded **protocol serialization support:** Adder now uses protobuf v1.36.12.

* Revised **Java workflow setup:** Windows signing and Java workflows now use `actions/setup-java` v5.7.0 with Java 21 unchanged.
...
* Renewed **Cardano test compatibility:** Adder now uses `ouroboros-mock` v0.15.1 for protocol testing.
