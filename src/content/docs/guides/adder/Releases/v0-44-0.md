...
* Extended **Cardano state access:** Adder now supports the `GetAccountState` query, handles operational certificates and Dijkstra redeemers, updates its Dijkstra era and Leios integration, and fixes validation-enabled pipeline handling so it surfaces an error and skips blocks that did not pass validation.

* Strengthened **Cardano validation:** Adder now applies stricter decoding and validation rules, uses upgraded decoding paths that safely tolerate duplicate pre-Conway MultiAsset map keys and Conway witness tagged-set entries, reports unknown inputs explicitly, and incorporates KES and ledger fixes.
