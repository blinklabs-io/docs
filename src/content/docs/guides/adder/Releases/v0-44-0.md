...
* Refined **tray notification rules:** The tray application now provides an in-app editor for notification rules, target lists, and preference checkboxes. It saves changes and applies them immediately without restarting Adder, while presenting recoverable errors more clearly.
...
* Strengthened **multi architecture image publishing:** Container releases now publish a shared image manifest so each supported platform receives its matching image.
...
* Outlined **troubleshooting paths:** A troubleshooting guide now covers connection, configuration, filter, push, webhook, error, and goroutine leak diagnostics.
...
* Extended **Cardano state access:** Adder now supports the `GetAccountState` query, handles operational certificates and Dijkstra redeemers, updates its Dijkstra era and Leios integration, and enables validation in the processing pipeline.

* Strengthened **Cardano validation:** Adder now applies stricter decoding and validation rules, rejects duplicate multi asset and witness data, reports unknown inputs explicitly, and incorporates KES and ledger fixes.
