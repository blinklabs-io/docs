...
Most configuration keys now also have matching CLI flags. Operators can choose a config file, environment variables, or CLI flags based on the deployment style. This example shows user visible settings that commonly move between those approaches, including `network`, `socketPath`, `metricsPort`, `storageMode`, and the API ports.
...
Dingo has a built in Mithril client that downloads and loads a snapshot automatically. This saves hours of sync time compared to replaying the chain from origin.
...
You should see log output showing the node connecting to peers, finishing any remaining recovery work, and syncing the remaining blocks until it reaches the chain tip.
