...
With token authentication, ordinary API requests require `Authorization: Bearer <token>`. Blockfrost also accepts the same token in its `project_id` header. Browser CORS preflight `OPTIONS` requests do not require a credential, but other requests still authenticate. Dingo reports invalid modes, missing or invalid certificate and key pairs, and token authentication without a `token` or `tokenFilePath` as startup configuration errors. The token file is the preferred credential source.
