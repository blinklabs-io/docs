...
> 📝 When switching `plugins.storage.metadata.provider` to `postgres`, `statementTimeout` limits each statement and `lockTimeout` limits lock acquisition waits; both use nonnegative millisecond values. For `mysql`, `statementTimeout` limits top-level read-only `SELECT` statements through `max_execution_time` in milliseconds, `lockTimeout` sets `innodb_lock_wait_timeout` in whole seconds and rounds subsecond values up, and `readTimeout` and `writeTimeout` set transport socket I/O deadlines. Each field defaults to `0`; Dingo rejects negative values and ignores all these fields when an explicit `dsn` is set.
