...
> 📝 Leave `debugPort` set to `0` unless profiling is required. `debugPort` remains disabled at `0`.

> 📝 Bark now derives its near tip safety window from the current ledger state.
