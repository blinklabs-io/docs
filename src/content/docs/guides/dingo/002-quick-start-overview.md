...
> 📝 When `plugins.storage.metadata.provider` is `postgres`, `statementTimeout` limits each statement and `lockTimeout` limits lock acquisition waits; both use nonnegative millisecond values. When the provider is `mysql`, `statementTimeout` limits top-level read-only `SELECT` statements through `max_execution_time` in milliseconds, `lockTimeout` sets `innodb_lock_wait_timeout` in whole seconds and rounds subsecond values up, and `readTimeout` and `writeTimeout` set transport socket I/O deadlines. Dingo defaults each field to `0`, rejects negative values, and ignores all these fields when an explicit `dsn` is set.
