...
* Added database snapshot, restore, and truncate commands for PostgreSQL, MySQL, S3, and GCS metadata and data. The commands validate before destructive resets, cloud backups use framed data, `snapshotDir` must be writable, and Docker database environments include UID/GID `1000:1000` and database client tools.
