...
* Adjusted OpenTelemetry dependencies so OTLP HTTP endpoints without a path no longer append `/v1/traces`; endpoint configurations may need an explicit signal path such as `/v1/traces`, Retry-After handling has changed, and OTLP HTTP trace export now supports `attribute.MAP` values.
