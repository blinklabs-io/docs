...
* Improved peer discovery safety by rejecting RFC 5737 TEST-NET and RFC 3849 documentation-only ranges for learned peers while preserving topology-private and established-inbound exceptions.
...
* Strengthened live chain rollback by limiting primary-chain rewinds to K, publishing rollback signals, classifying over-K and Mithril cases for resynchronization, preserving durable undo ordering, and guarding buffered headers.
...
* Expanded ArchiveService lookup options to support block references by hash, slot, and height, return `not_found` for each missing reference, use metadata for height lookup, decode compact metadata, and seek through S3 data within bounded limits.

* Ordered chain-selection events through dedicated lanes, added a watchdog for stalled event handlers, and exposed the `event_subscriber_handler_stalled_total` metric.
...
* Introduced bounded Leios failover that responds to context cancellation, recycles dead connections, deduplicates certified endorser-block recovery, applies escalating backoff, and updates callback signatures.
