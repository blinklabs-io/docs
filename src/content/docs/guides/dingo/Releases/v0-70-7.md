...
* Narrowed zero produced output warnings to dropped declared outputs and collateral returns, and added specific log attributes for each case.

* Classified one sided zero value Koios reward rows as informational agreement, compared only rewards credited by Dingo, and reported significant mismatch counts.

* Withheld Dingo’s synthetic PlutusV2 cost model from external current protocol parameter responses until real provenance exists, and persisted that marker across restarts, rollbacks, and truncation.

* Verified consistent sigma numerator and denominator reads across snapshot recapture with deterministic regression coverage.

* Confirmed that Dijkstra required redeemer validation still runs when phase 2 is skipped.

* Raised the `golang.org/x/crypto` dependency from `0.55.0` to `0.56.0`.

* Strengthened rollback and rewind recovery by waiting for batch commits before restoring indexes, recording skipped restores, and validating recovery targets against chain membership.

* Added per epoch Koios protocol parameter parity through the `koios_epoch_params` cache, including backfill and result classification behavior.
...
* Gated forging with an operational certificate counter preflight for each era and aligned the check with ledger validation.

* Separated service backed and race test jobs in release CI.

* Enforced aggregate inbound block `MaxBlockExUnits`, counted Dijkstra subtransaction witness budgets, and aligned invalid transaction overlay semantics.

* Delayed read result completion until cached boundary batches were applied and delegated cold operational certificate verification to the shared verifier.
...
* Updated the AWS SDK for Go S3 integration from `1.107.3` to `1.110.0`.
