...
* Improved transaction lookup batching for MySQL and Postgres so UTxO reads complete with fewer round trips.

* Added MySQL database name validation before database creation so invalid names fail early.

* Expanded the Blockfrost API with asset holder address lookups.

### 🧪 Testing, Docs, and Tooling

* Added direct tests for ledger delta application to cover `LedgerDelta.apply` behavior.
