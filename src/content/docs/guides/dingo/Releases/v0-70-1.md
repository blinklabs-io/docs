...
* Closed immutable chunk readers on every block lookup return path, enabling Windows cleanup and preventing descriptor leaks.
...
* Preserved prior Koios parity mismatch evidence when checks fail by replacing mismatch data only after all fallible reads succeed.
