...
* Bounded restrictive phase-2 Plutus CEK evaluation by each protocol parameter set's per-transaction `MaxTxExUnits` across Alonzo, Babbage, and Conway, while continuing to compare declared redeemer budgets and account for trailing slippage batches.
...
* Aligned committee voting sets so tallies count only seated, non-expired, non-resigned members with active hot-key authorization; `ExpiresEpoch` is inclusive and unauthorized votes are ignored.
