...
* Fixed the archive fallback so Dingo verifies the requested block slot and hash before it accepts a downloaded block.

* Verified archived blocks locally so Dingo derives metadata from the decoded block and rejects any block type that does not match the decoded era.

* Strengthened archive error handling with explicit failures for undecodable blocks, hash mismatches, slot mismatches, metadata mismatches, block type mismatches, and non fully authenticated Byron main reads.
...
* Clarified Bark archive trust boundaries and verification rules in the architecture and database documentation.

* Updated the bundled `gouroboros` dependency to v0.190.0.

* Aligned leader schedule and header verification with consistent error returns.
