...
* Improved **run history expiry as a standalone storage option:** Dingo now lets operators expire older local block history without relying on archive fallback, while still keeping Bark archive access as a separate choice for deployments that need it.

### 🔧 Fixes

* Corrected **validate ledger fees more accurately for Conway transactions:** Dingo now checks minimum fees more accurately, including reference script costs and required redeemer handling, which reduces avoidable transaction validation failures.
...
* Documented **publish the repository v0.51.0 release notes:** The repository release notes now include the v0.51.0 page, which keeps recent release history current.
