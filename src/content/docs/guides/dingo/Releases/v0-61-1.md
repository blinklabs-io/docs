...
* Improved **keep metadata plugin builds more selective:** Dingo now keeps MySQL and Postgres metadata plugins opt in behind the `dingo_extra_plugins` build tag, which gives operators tighter control over optional metadata plugin support when building Dingo.
