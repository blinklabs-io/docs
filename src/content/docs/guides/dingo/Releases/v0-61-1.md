...
* Improved **keep metadata plugin builds more selective:** Dingo now keeps MySQL and Postgres metadata plugins opt in behind the `dingo_extra_plugins` build tag, which gives operators tighter control over optional metadata plugin support.
...
* Modernized **avoid duplicate Leios endorser transactions:** Dingo now skips duplicate endorser transactions, which keeps repeated submissions out of Leios recovery flows.
...
* Verified **check inbound block leadership eligibility more accurately:** Dingo now verifies slot leader VRF eligibility on inbound blocks against stake derived thresholds, which improves ledger verification reliability.

* Normalized **verify blocks from canonical VRF data:** Dingo now normalizes the VRF key and result from the canonical block header data during block verification, which improves recovery and verification reliability.
