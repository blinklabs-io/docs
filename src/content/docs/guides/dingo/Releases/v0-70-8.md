...
* Improved chain synchronization by publishing genesis selection state as one consistent snapshot without holding the selector lock during active synchronization, preventing a deadlock from permanently stopping synchronization.

* Improved validation reliability by keeping Plutus cost model state separate between evaluations, preventing concurrent validation races across the root and Antithesis modules.
