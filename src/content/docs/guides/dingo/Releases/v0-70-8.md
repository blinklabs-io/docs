...
* Serialized `Node.Stop` with active `Restore`, `Truncate`, and `Snapshot` operations under one shutdown deadline, canceled early, waited for lifecycle checkpoints, and kept shutdown retryable after a timeout while waiting for a checkpoint.
