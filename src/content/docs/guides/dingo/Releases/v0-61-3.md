...
* Updated `examples/dingo-sundae-preview` from Vite 8.1.0 to 8.1.3, including the Vite 8.1.3 fixes.

* Improved `examples/dingo-blockfrost-explorer` from Vite 8.1.0 to 8.1.3, including the Vite 8.1.3 fixes.
...
* Added `google.golang.org/grpc` from 1.81.1 to 1.82.0.

* Clarified that strict path checking stays enabled even when `GRPC_GO_EXPERIMENTAL_DISABLE_STRICT_PATH_CHECKING` is set, and load-balancing policy registry lookup now treats names as case-sensitive.
