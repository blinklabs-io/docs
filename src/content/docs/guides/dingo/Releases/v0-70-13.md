...
* Added caching for UTxO inserts and asset ID lookups in SQLite and PostgreSQL while preserving MySQL `RETURNING` behavior through dialect-aware handling.

* Enhanced chain selection with a `2 second` switch-back cooldown, preserved healthy in-flight block fetch batches across fork-extension restarts, and resynchronized after unrecoverable restart failures.

* Consolidated reward live stake refreshes so each unique stake credential refreshes only once per transaction.

* Replaced the local DMQ phase 2 authenticator with the `gouroboros` message authenticator, removed `dmq.Authenticator` and `NewAuthenticator`, and reserved node wiring for a future update.

* Documented environment-specific Mithril API mode bootstrap measurements for Preprod and Preview, including a benchmark result of approximately `15%` to `30%` and a caveat that the values establish a baseline rather than a guarantee.

* Validated Shelley pointer address payloads during UTxO parsing and rejected malformed pointers.

* Expanded deterministic Leios fetch regression coverage to include deadline boundaries.

* Updated Antithesis scripts to obtain stake address key hashes with the latest `cardano-cli` command.
