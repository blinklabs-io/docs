...
|KES key<br> (kes.skey / kes.vkey)	|Signs blocks; rotated before KES expiration	|Local `kes.skey` on the block producer when the node uses `shelleyKesKey`; external Bursa agent when the node uses `shelleyKesAgentSocket`
