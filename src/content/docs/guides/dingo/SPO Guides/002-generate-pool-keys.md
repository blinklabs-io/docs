...
|KES key<br> (kes.skey / kes.vkey)	|Signs blocks; rotated before KES expiration	|Local `kes.skey` on the block producer when `shelleyKesKey` is configured; external Bursa agent when `shelleyKesAgentSocket` is configured
