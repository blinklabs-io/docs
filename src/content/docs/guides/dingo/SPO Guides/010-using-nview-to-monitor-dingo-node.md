...
Simply download the `nview` binary file from blinklabs.io onto your node server. Then run `nview` in the server command line. It's that simple to use, and you will get monitoring right out of the box!
...
For this example, we named the binary file `nview` and saved it to our `$DINGO_HOME` directory. 
