...
> 📝 When `plugins.storage.metadata.provider` is `postgres`, `statementTimeout` limits each statement and `lockTimeout` limits lock acquisition waits; these fields accept duration values such as `30s`. PostgreSQL converts positive duration values to the `statement_timeout` and `lock_timeout` session settings, which use milliseconds. When the provider is `mysql`, `statementTimeout` limits top-level read-only `SELECT` statements through `max_execution_time` in milliseconds, `lockTimeout` sets `innodb_lock_wait_timeout` in whole seconds and rounds subsecond durations up, and `readTimeout` and `writeTimeout` use duration values as transport socket I/O deadlines. Dingo defaults each field to `0`, rejects negative values, and ignores all these fields when an explicit `dsn` is set.
