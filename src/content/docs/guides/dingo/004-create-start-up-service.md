...
> 📝 Current releases can change runtime behavior in a few practical ways. Near the tip, Dingo uses median peer latency when enough samples exist before it starts shadow blockfetch from a second peer. The slot clock now tolerates up to `500ms` of drift by default, which reduces noisy timing warnings on busy systems. Ledger processing now recovers more cleanly if nonce data for leader checks is missing. Forging now handles TPraos era boundaries such as Shelley to Allegra correctly, and it also supports pre-Conway eras.
