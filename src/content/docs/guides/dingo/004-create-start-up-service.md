...
> 📝 Operators upgrading an older service config should remove any `barkPrunerFrequency` entry from `/etc/dingo/dingo.yaml` and use `historyExpiry.enabled` and `historyExpiry.frequency` instead.
