...
Current releases derive the Bark safety window from ledger state, so operators no longer set a manual Bark security window value.
...
> ⚠️ `debugPort` controls a separate optional profiling endpoint, not the `metricsPort` endpoint. Leave it at `0` unless temporary profiling or debugging requires it.
...
> 📝 Current releases can change runtime behavior in a few practical ways. Near the tip, Dingo now compares peer latency samples before it sends the same block request to a second peer, which helps it avoid extra duplicate work when the primary peer is already responding quickly. The slot clock now tolerates up to `500ms` of drift by default, which reduces noisy timing warnings on busy systems. Ledger processing now recovers more cleanly if nonce data for leader checks is missing. Forging now handles era boundaries such as Shelley to Allegra correctly, and it also supports pre-Conway eras.
