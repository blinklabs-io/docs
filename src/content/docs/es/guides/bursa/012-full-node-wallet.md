...
En Linux, instalar las cabeceras de desarrollo de `webkit2gtk` antes de ejecutar el destino. Si el sistema solo ofrece `webkit2gtk-4.1`, el archivo Makefile crea la adaptación `pkg-config` necesaria para la dependencia que solicita `4.0`.
...
El binario con webview necesita `webkit2gtk` y sus cabeceras de desarrollo. Instalar esa dependencia y volver a ejecutar `make wallet-webview`. Como alternativa, compilar el binario puro de Go y abrir `http://127.0.0.1:8090` en un navegador:
