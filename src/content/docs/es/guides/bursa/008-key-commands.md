...
Al cargar una clave desde un archivo, Bursa rechaza cualquier entrada que no sea un archivo regular. En `Unix`, rechaza las entradas que usan enlaces simbólicos; en `Windows`, rechaza los puntos de reanálisis. Bursa limita la entrada al máximo admitido por la implementación y rechaza los archivos sobredimensionados antes de procesarlos. La ruta de clave pública permite seguir leyendo los archivos públicos. Bursa aplica la comprobación de permisos de propietario únicamente cuando carga material secreto.
