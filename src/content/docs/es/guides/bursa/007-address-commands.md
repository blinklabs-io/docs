...
Bursa acepta `mainnet`, `preprod` y `preview` como valores de red. Las solicitudes de API afectadas (`POST /api/script/create`, `POST /api/script/address` y `POST /api/address/build`) rechazan `testnet` con un error de validación HTTP 400.
- Direcciones base (credenciales de pago + stake)
```bash
