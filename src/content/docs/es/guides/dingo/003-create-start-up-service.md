...
> 📝 `databaseLifecycle.snapshotRetention` conserva los snapshots automáticos más recientes. `databaseLifecycle.snapshotCloudDestination` refleja cada snapshot en S3 o GCS cuando Dingo se compila con `dingo_extra_plugins`. `databaseLifecycle.snapshotEnabled` no admite capturas automáticas cuando `badger`, `s3` o `gcs` es el proveedor principal de blobs. Las operaciones manuales `dingo database snapshot` y `CreateSnapshot` de Bark siguen disponibles.
