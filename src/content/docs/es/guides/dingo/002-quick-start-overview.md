...
> 📝 Para que Midnight sirva por gRPC, configure `storageMode: "api"`, `midnight.serverEnabled: true` y un valor distinto de `0` en `midnight.port`. La indexación y el servicio son controles independientes. Dingo mantiene deshabilitada la reflexión hasta que se active `midnight.reflectionEnabled: true`; esta opción requiere `midnight.serverEnabled: true`. Dingo mantiene el servidor gRPC deshabilitado cuando `midnight.serverEnabled` es `false`. `127.0.0.1` es el host predeterminado, incluso cuando `midnight.host` está vacío. Dingo limita el texto plano al loopback; para exponerlo fuera del loopback, configure `midnight.allowInsecureRemote: true`, o configure `tlsCertFilePath` y `tlsKeyFilePath` para una exposición remota protegida.
