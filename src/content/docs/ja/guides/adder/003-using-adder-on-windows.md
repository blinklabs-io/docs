...
同じグループ内の値はOR条件で照合します。複数のグループを登録した場合は、入力済みグループ間のコネクタで`OR`または`AND`を選択できます。`OR`はどれか1つのグループに一致すれば監視し、`AND`は接続されたグループの条件をすべて満たす場合に監視します。
...
WindowsでAdder Trayが使用する設定ファイルの保存先は`%APPDATA%\Adder\adder-tray.yaml`です。以前の設定でAdder Trayのフィルター設定が未設定の場合、アップグレード時に`engine.yaml`の旧`filter.cardano`対象キー（`address`、`drep`、`pool`、`asset`、`policy`）を対応する対象リストへ移行します。新しい設定を適用すると、`engine.yaml`からこれらの旧対象キーを削除します。
