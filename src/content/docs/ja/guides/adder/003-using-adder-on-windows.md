...
エディターは、同じ対象を重複して登録したルールや成立しない条件を拒否します。入力を修正してからもう一度`Apply & Restart`を選択します。
...
`Apply`の後に警告が表示された場合も、設定は保存されています。サービスの再起動またはAdder APIへの再接続が完了しない場合、ルールエディターは開いたまま入力を再び有効にします。状態と`adder-tray.log`を確認し、接続が復旧してから設定を再試行します。
