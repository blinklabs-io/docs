...
| フィールド | 型 | 必須条件 | 説明 |
| --- | --- | --- | --- |
| `everything` | 真偽値 | `true` または対象配列のいずれかが必要 | `true` の場合、Adder は対象配列を無視して、すべての対象を監視します。 |
...
| `eventType` | 文字列 | 通知ルールが通知の契機に使ったイベント種別です。値がない場合、Adder はこのフィールドを省略します。 |
| `title` | 文字列 | 通知のタイトルです。ルールにタイトルがない場合は `Adder` になります。 |
| `body` | 文字列 | 通知本文です。 |
| `batched` | 真偽値 | 通知が複数イベントをまとめたものかどうかを示します。 |
| `count` | 整数 | Adder が通知にまとめたイベント数です。 |
...
`notify-json` は対象を認識する通知ルールとレート制限を使用します。Adder はチェーンイベントを通知ルールで評価する前に、安定した JSON のフィールド名と配列・マップの形式へ正規化します。
...
3. Adder が設定ファイルを読み込み、JSON としての正しさ、`schemaVersion` 1、すべての設定制約を確認できること。
