...
> サービスアカウント JSON ファイルのパスを指定。空でないパスが必要で、JSON には空でない文字列の `project_id` が必要です。
>
> FCM HTTP 送信は 10 秒でタイムアウトします。認証情報の読み込みまたは検証、アクセストークンの取得、メッセージの作成、送信に失敗した場合、エラーは Adder のパイプラインに通知され、エラーログに表示されます。
<br />
