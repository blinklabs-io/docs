...
**症状**: Adderは接続を確立した後、`handshake failed: network magic mismatch`などのエラーを記録してプロトコルのハンドシェイクを中断します。
...
Adderは同じ設定を複数の方法で指定すると、次の順序で値を適用します。
...
Adderは異なる種類のフィルター（`type`と`address`など）をAND条件で評価します。同じ種類のフィルターに複数の値を指定すると、AdderはOR条件で評価します。
