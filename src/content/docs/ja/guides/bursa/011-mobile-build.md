...
スモークテストはarm64ランナー上のAPI 34、`google_apis`、`arm64-v8a`エミュレータで実行します。テストはビルドジョブが生成した署名済みAPKを起動し、埋め込みウォレットがready状態になり、`WalletService`を確認できることを検証します。APKがこの検証を満たさない場合、スモークテストは失敗します。
