...
鍵ファイルを読み込むときは通常ファイルを使用してください。Bursaは通常ファイル以外を拒否し、UnixのシンボリックリンクとWindowsの再解析ポイントも拒否します。入力サイズは実装がサポートする最大値までに制限され、上限を超える鍵ファイルは処理を開始する前に拒否されます。公開鍵ファイルは公開鍵の読み込み経路で引き続き読み込めます。秘密鍵マテリアルを実際に読み込む場合にだけ、上記の所有者専用権限チェックを適用します。公開 `.vkey` ファイルや運用証明書などの公開アーティファクトには適用しません。
...
VRF検証鍵には期待される長さが必要です。VRF署名鍵には、シード単体またはシードと公開鍵を連結した形式を使用できます。シードと公開鍵の形式では、Bursaがシードから導出した公開鍵と埋め込まれた公開鍵を照合します。長さが不正な鍵、シードと公開鍵が一致しない鍵は、Bursaがエンベロープを生成せずに拒否します。
...
KES検証鍵には期待される長さが必要です。KES署名鍵にはCardano KES depthとKES secret-key sizeの要件があります。長さが不正な検証鍵、無効なKES depth、または無効なKES secret-key sizeを含む鍵は、Bursaがエンベロープを生成せずに拒否します。
