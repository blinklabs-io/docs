...
ハッシュは、入力ファイルから読み取った正確なバイト列をBlake2b-256で計算します。JSONとして検証しますが、意味的に同等なJSONでも空白、フィールド順序、書式が異なるとハッシュが変わる場合があります。正規JSONが必要な場合は、コマンドを実行する前に、正規化したJSONを入力ファイルまたはホスト先ファイルに保存してください。
