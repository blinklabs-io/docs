...
## ステップ3 - Mithrilアーティファクトからのブートストラップ（初回実行のみ）

サービスを初めて起動する前に、Mithrilアーティファクトからデータベースをブートストラップします：
...
これによりMithrilアーティファクトをダウンロードしてデータベースにロードし、数時間の同期時間を節約できます。詳細は[クイックスタートガイドのステップ4：Mithrilアーティファクトからのブートストラップ](../002-quick-start-overview#ステップ4---mithrilスナップショットからのブートストラップ)を参照してください。
