...
特定のMithrilアーティファクトを選択するには、YAMLの `mithril.pinnedDigest`、CLIの `--mithril-pinned-digest`、または環境変数 `DINGO_MITHRIL_PINNED_DIGEST` でピンを指定できます。この指定は新しいデータベースの初回ブートストラップにのみ使用でき、完全なデータベースのキャッチアップで明示的なピンを指定するとDingoは処理を拒否します。中断したインポートを再開する場合は、保存済みのアーティファクト識別子を使用する必要があり、別のピンで上書きすることはできません。
