...
`--mode`には`full`または`incremental`を指定します。既定値は`incremental`です。以前の全体比較の動作が必要な場合は、`--mode full`を明示的に指定します。これ以外の値を指定すると、`watch`はエラーを返します。
...
- `--full-check-interval`: 全体比較を実行する間隔をブロック数で指定します。既定値は`1000`です。`0`または負の値は指定できません。
