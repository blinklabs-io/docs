...
> 💡 API サーバーは `storageMode: "api"` のときだけ有効です。Blockfrost互換の `pools` 系エンドポイントも利用できます。各 API の `port` を `0` にすると、その API は無効になります。
...
> 📝 `mithril.verifyCertificates: true` を有効にすると、Dingoは ancillary ledger state と設定ネットワークの一致も確認します。
