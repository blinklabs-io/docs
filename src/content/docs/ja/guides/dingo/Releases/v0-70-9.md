...
* 追加: `AcquireSpecificPoint`によるpoint-aware LocalStateQueryに対応し、要求されたpointが現在のchain上にあることを検証して、対応する過去のpool distribution、protocol parameter、epoch queryで使用するようにしました。利用できない、または未対応の過去データは専用エラーを返し、`GetUTxOWhole`はlive stateのみを対象とします。connection終了時には取得したpoint stateを解放します。
...
* 拡張: point固定LocalStateQueryで、過去のstake distribution、protocol parameter、transaction別UTxOの再構成に対応しました。queryはslot時点のstateを使用し、履歴が利用できない場合はfail closedになり、block admissionとforgingではreference scriptのaggregate budgetを適用します。
